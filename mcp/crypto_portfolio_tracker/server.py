"""
crypto_portfolio_tracker — ONE unified MCP server forged by FORGE.
Generated : 2026-08-20 10:41 UTC
Goal      : Track top crypto prices and alert via Slack and Notion
Forged    : coingecko.com
Wrapped   : Notion, Slack
...
CONFIG ONCE — then just say: "Use crypto_portfolio_tracker at <absolute path to this file>"
"""
...
mcp = FastMCP("crypto_portfolio_tracker")
...
TOOL_MANIFEST = [
    ...
    },
    {
        ...
        "source": "Custom coingecko.com Forged",
        "badge": "FORGED",
        "description": "Open coingecko.com, type a query into the search box and return the results page text."
    },
    {
        "name": "read_page_coingecko",
        "source": "Custom coingecko.com Forged",
        "badge": "FORGED",
        "description": "Open coingecko.com and return the main readable page content."
    },
    {
        "name": "click_element_coingecko",
        "source": "Custom coingecko.com Forged",
        "badge": "FORGED",
        "description": "Open coingecko.com and click the button whose visible text matches element_text (role locator first, CSS fallback)."
    },
    {
        "name": "fill_field_coingecko",
        "source": "Custom coingecko.com Forged",
        "badge": "FORGED",
        "description": "Open coingecko.com and type a value into its main input field, then press Enter."
    },
    {
        "name": "extract_links_coingecko",
        "source": "Custom coingecko.com Forged",
        "badge": "FORGED",
        "description": "Open coingecko.com and return the visible links on the page."
    },
    {
        "name": "slack_post_message",
        "source": "Official Slack",
        "badge": "OFFICIAL",
        "description": "Post a message to a Slack channel (#general or channel id)"
    },
    {
...
# ---- forged for coingecko.com (https://coingecko.com) ----


@mcp.tool()
def search_coingecko(query: str, limit: int = 10) -> list:
    """Open coingecko.com, type a query into the search box and return the results page text."""
    page = _ensure_page()
    try:
        page.goto("https://coingecko.com", wait_until="domcontentloaded", timeout=30000)
        ...
        return [{"ok": False, "tool": "search_coingecko", "error": repr(err)}]


@mcp.tool()
def read_page_coingecko(limit: int = 8) -> list:
    """Open coingecko.com and return the main readable page content."""
    page = _ensure_page()
    try:
        page.goto("https://coingecko.com", wait_until="domcontentloaded", timeout=30000)
        page.wait_for_timeout(2000)
        ...
        return [{"ok": False, "tool": "read_page_coingecko", "error": repr(err)}]


@mcp.tool()
def click_element_coingecko(element_text: str) -> list:
    """Open coingecko.com and click the button whose visible text matches element_text (role locator first, CSS fallback)."""
    page = _ensure_page()
    try:
        page.goto("https://coingecko.com", wait_until="domcontentloaded", timeout=30000)
        page.wait_for_timeout(1500)
        ...
        return [{"ok": False, "tool": "click_element_coingecko", "error": repr(err)}]


@mcp.tool()
def fill_field_coingecko(value: str) -> list:
    """Open coingecko.com and type a value into its main input field, then press Enter."""
    page = _ensure_page()
    try:
        page.goto("https://coingecko.com", wait_until="domcontentloaded", timeout=30000)
        page.wait_for_timeout(1500)
        ...
        return [{"ok": False, "tool": "fill_field_coingecko", "error": repr(err)}]


@mcp.tool()
def extract_links_coingecko(limit: int = 15) -> list:
    """Open coingecko.com and return the visible links on the page."""
    page = _ensure_page()
    try:
        page.goto("https://coingecko.com", wait_until="domcontentloaded", timeout=30000)
        page.wait_for_timeout(1500)
        ...
        return [{"ok": False, "tool": "extract_links_coingecko", "error": repr(err)}]
...
def slack_post_message(channel: str, text: str = "") -> dict:
    """Post a message to a Slack channel (#general or channel id) (official Slack wrapper; needs SLACK_TOKEN)"""
    import httpx

    token = os.environ.get("SLACK_TOKEN", "")
    if not token:
        return {"ok": False, "error": "env SLACK_TOKEN not set — add it to the env block of your mcpServers config"}
    try:
