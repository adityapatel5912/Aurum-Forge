"""
stress_7 — ONE unified MCP server forged by FORGE.
Generated : 2026-08-17 15:57 UTC
Goal      : Track price site 7 and log to notion
Forged    : example.com
Wrapped   : Notion
...
CONFIG ONCE — then just say: "Use stress_7 at <absolute path to this file>"
"""
...
TOOL_MANIFEST = [
    ...
    },
    {
        ...
        "badge": "FORGED",
        "description": "Open example.com, type a query into the search box and return the results page text."
    },
    {
        "name": "read_page_example",
        "source": "Custom example.com Forged",
        "badge": "FORGED",
        "description": "Open example.com and return the main readable page content."
    },
    {
        "name": "click_element_example",
        "source": "Custom example.com Forged",
        "badge": "FORGED",
        "description": "Open example.com and click the button whose visible text matches element_text (role locator first, CSS fallback)."
    },
    {
        "name": "fill_field_example",
        "source": "Custom example.com Forged",
        "badge": "FORGED",
        "description": "Open example.com and type a value into its main input field, then press Enter."
    },
    {
        "name": "extract_links_example",
        "source": "Custom example.com Forged",
        "badge": "FORGED",
        "description": "Open example.com and return the visible links on the page."
    },
    {
...
# ---- forged for example.com (https://example.com) ----


@mcp.tool()
def search_example(query: str, limit: int = 10) -> list:
    """Open example.com, type a query into the search box and return the results page text."""
    page = _ensure_page()
    try:
        ...
        return [{"ok": False, "tool": "search_example", "error": repr(err)}]


@mcp.tool()
def read_page_example(limit: int = 8) -> list:
    """Open example.com and return the main readable page content."""
    page = _ensure_page()
    try:
        page.goto("https://example.com", wait_until="domcontentloaded", timeout=30000)
        page.wait_for_timeout(2000)
        ...
        return [{"ok": False, "tool": "read_page_example", "error": repr(err)}]


@mcp.tool()
def click_element_example(element_text: str) -> list:
    """Open example.com and click the button whose visible text matches element_text (role locator first, CSS fallback)."""
    page = _ensure_page()
    try:
        page.goto("https://example.com", wait_until="domcontentloaded", timeout=30000)
        page.wait_for_timeout(1500)
        ...
        return [{"ok": False, "tool": "click_element_example", "error": repr(err)}]


@mcp.tool()
def fill_field_example(value: str) -> list:
    """Open example.com and type a value into its main input field, then press Enter."""
    page = _ensure_page()
    try:
        page.goto("https://example.com", wait_until="domcontentloaded", timeout=30000)
        page.wait_for_timeout(1500)
        ...
        return [{"ok": False, "tool": "fill_field_example", "error": repr(err)}]


@mcp.tool()
def extract_links_example(limit: int = 15) -> list:
    """Open example.com and return the visible links on the page."""
    page = _ensure_page()
    try:
        page.goto("https://example.com", wait_until="domcontentloaded", timeout=30000)
        page.wait_for_timeout(1500)
        ...
        return [{"ok": False, "tool": "extract_links_example", "error": repr(err)}]
