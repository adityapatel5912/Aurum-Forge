...
Uses MCP Server: research_v5 with 12 tools
...
You have MCP Server `research_v5` installed. Follow this exact sequence:
...
2. IF result meets threshold (discount >20%, prize>50000, price<2000):
   - CALL `gmail_send_email` (from research_v5) in parallel
   - CALL `gmail_notify_and_log` (from research_v5) in parallel
3. Return summary

Do not re-discover tools — use research_v5 tools directly. This SKILL.md is single source of truth for this workflow.
