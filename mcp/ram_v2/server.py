"""
ram_v2 — ONE unified MCP server forged by FORGE.
Generated : 2026-08-18 11:58 UTC
Goal      : Forge RAM tracker MCP and email alerts on discount >20%
Forged    : amazon.com
Wrapped   : none
...
CONFIG ONCE — then just say: "Use ram_v2 at <absolute path to this file>"
"""
...
TOOL_MANIFEST = [
    ...
    {
        "name": "search_amazon",
        "source": "Custom amazon.com Forged",
        "badge": "FORGED",
        "description": "Open amazon.com, type a query into the search box and return the results page text."
    },
    {
        "name": "read_page_amazon",
        "source": "Custom amazon.com Forged",
        "badge": "FORGED",
        "description": "Open amazon.com and return the main readable page content."
    },
    {
        "name": "click_element_amazon",
        "source": "Custom amazon.com Forged",
        "badge": "FORGED",
        "description": "Open amazon.com and click the button whose visible text matches element_text (role locator first, CSS fallback)."
    },
    {
        "name": "fill_field_amazon",
        "source": "Custom amazon.com Forged",
        "badge": "FORGED",
        "description": "Open amazon.com and type a value into its main input field, then press Enter."
    },
    {
        "name": "extract_links_amazon",
        "source": "Custom amazon.com Forged",
        "badge": "FORGED",
        "description": "Open amazon.com and return the visible links on the page."
    }
]
...
# ---- forged for amazon.com (https://amazon.com) ----


@mcp.tool()
def search_amazon(query: str, limit: int = 10) -> list:
    """Open amazon.com, type a query into the search box and return the results page text."""
    page = _ensure_page()
    try:
        page.goto("https://amazon.com", wait_until="domcontentloaded", timeout=30000)
        ...
        return [{"ok": False, "tool": "search_amazon", "error": repr(err)}]


@mcp.tool()
def read_page_amazon(limit: int = 8) -> list:
    """Open amazon.com and return the main readable page content."""
    page = _ensure_page()
    try:
        page.goto("https://amazon.com", wait_until="domcontentloaded", timeout=30000)
        page.wait_for_timeout(2000)
        ...
        return [{"ok": False, "tool": "read_page_amazon", "error": repr(err)}]


@mcp.tool()
def click_element_amazon(element_text: str) -> list:
    """Open amazon.com and click the button whose visible text matches element_text (role locator first, CSS fallback)."""
    page = _ensure_page()
    try:
        page.goto("https://amazon.com", wait_until="domcontentloaded", timeout=30000)
        page.wait_for_timeout(1500)
        ...
        return [{"ok": False, "tool": "click_element_amazon", "error": repr(err)}]


@mcp.tool()
def fill_field_amazon(value: str) -> list:
    """Open amazon.com and type a value into its main input field, then press Enter."""
    page = _ensure_page()
    try:
        page.goto("https://amazon.com", wait_until="domcontentloaded", timeout=30000)
        page.wait_for_timeout(1500)
        ...
        return [{"ok": False, "tool": "fill_field_amazon", "error": repr(err)}]


@mcp.tool()
def extract_links_amazon(limit: int = 15) -> list:
    """Open amazon.com and return the visible links on the page."""
    page = _ensure_page()
    try:
        page.goto("https://amazon.com", wait_until="domcontentloaded", timeout=30000)
        page.wait_for_timeout(1500)
        ...
        return [{"ok": False, "tool": "extract_links_amazon", "error": repr(err)}]
