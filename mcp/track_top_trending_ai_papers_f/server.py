"""
track_top_trending_ai_papers_f — ONE unified MCP server forged by FORGE.
Generated : 2026-08-20 12:10 UTC
Goal      : Track top trending AI papers from ArXiv, extract key findings, and create structured research briefs in Notion and notify via Slack
Forged    : arxiv.org
...
CONFIG ONCE — then just say: "Use track_top_trending_ai_papers_f at <absolute path to this file>"
"""
...
mcp = FastMCP("track_top_trending_ai_papers_f")
...
# ---- forged for arxiv.org (https://arxiv.org/list/cs.AI/recent) ----


@mcp.tool()
def search_arxiv(query: str, limit: int = 10) -> list:
    """Open arxiv.org, search for research papers matching query and return extract."""
    page = _ensure_page()
    try:
        import urllib.parse
        encoded = urllib.parse.quote_plus(query)
        search_url = f"https://arxiv.org/search/?query={encoded}&searchtype=all&source=header"
        page.goto(search_url, wait_until="domcontentloaded", timeout=30000)
        page.wait_for_timeout(2000)
        return _extract(page, css="ol.bidi-override li, .arxiv-result, body", limit=limit)
    except Exception as err:
