"""
track_real_time_currency_excha — ONE unified MCP server forged by FORGE.
Generated : 2026-08-20 11:45 UTC
...
CONFIG ONCE — then just say: "Use track_real_time_currency_excha at <absolute path to this file>"
"""
...
mcp = FastMCP("track_real_time_currency_excha")
...
TOOL_MANIFEST = [
    ...
    {
        "name": "search_xe",
        "source": "Custom xe.com Forged",
        "badge": "FORGED",
        "description": "Open xe.com, type a query into the search box and return the results page text."
    },
    {
        "name": "read_page_xe",
        "source": "Custom xe.com Forged",
        "badge": "FORGED",
        "description": "Open xe.com and return the main readable page content."
    },
    {
...
# ---- forged for xe.com (https://www.xe.com) ----


@mcp.tool()
def search_xe(query: str, limit: int = 10) -> list:
    """Open xe.com, type a query into the search box and return the results page text."""
    page = _ensure_page()
    try:
        page.goto("https://www.xe.com", wait_until="domcontentloaded", timeout=30000)
        page.wait_for_timeout(1500)
        _smart(page, "fill", role="searchbox", name="Search", css="input[type='search']", value=query)
        _smart(page, "click", role="button", name="Current language: EN", css="button.focus\\:outline-xe-primary-500.flex")
        page.wait_for_timeout(2000)
        return _extract(page, css="body", limit=12)
    except Exception as err:
        return [{"ok": False, "tool": "search_xe", "error": repr(err)}]


@mcp.tool()
def read_page_xe(limit: int = 8) -> list:
    """Open xe.com and return the main readable page content."""
    page = _ensure_page()
    try:
        page.goto("https://www.xe.com", wait_until="domcontentloaded", timeout=30000)
        page.wait_for_timeout(2000)
        ...
        return [{"ok": False, "tool": "read_page_xe", "error": repr(err)}]
