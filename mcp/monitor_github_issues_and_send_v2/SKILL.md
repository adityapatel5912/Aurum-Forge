...
Uses MCP Server: monitor_github_issues_and_send_v2 with 24 tools
...
You have MCP Server `monitor_github_issues_and_send_v2` installed. Follow this exact sequence:
...
2. IF result meets threshold (discount >20%, prize>50000, price<2000):
   - CALL `telegram_send_message` (from monitor_github_issues_and_send_v2) in parallel
   - CALL `notion_create_database_entry` (from monitor_github_issues_and_send_v2) in parallel
3. Return summary

Do not re-discover tools — use monitor_github_issues_and_send_v2 tools directly. This SKILL.md is single source of truth for this workflow.
