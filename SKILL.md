...
- **Antigravity**: Add `chain_sales_outreach` to `~/.gemini/antigravity/mcp_config.json` or `~/.antigravity/mcp.json`
- **Codex**: Run `codex mcp add chain_sales_outreach -- python "mcp/chain_sales_outreach/server.py"` or add to `~/.codex/config.json`
- **Z Code**: Add `chain_sales_outreach` to `settings.json` under `context_servers` or `~/.zcode/mcp.json`
